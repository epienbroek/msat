...
#
#   msat_ls_rpm_sc.py is free software; you can redistribute
#   it and/or modify it under the terms of the GNU General
#   Public License as published by the Free Software
#   Foundation; either version 3 of the License, or (at your
#   option) any later version.
#
#   msat_ls_rpm_sc.py is distributed in the hope that it
...
usage = '''list RPM and containing software channel'''

description = '''This script lists the specified RPM and the containing software channels. Note, searches for RPM pattern name.'''
...
parser.add_option(
  ...
  type = "string",
  default = None,
  help = "RPM name pattern to search for",
)

(options, args) = config.get_conf(parser)

if not options.rpm_pattern:
  print >> sys.stderr, "Error: must provide RPM name search pattern, -n or --rpm-pattern"
  sys.exit(1)
...
try:
  rpms = client.packages.search.advanced(
    key,
    'name:%s' % (options.rpm_pattern, ),
  )
...
for r in rpms:
  try:
    ...
    print >> sys.stderr, str(e)
    print >> sys.stderr, id
  print '%s %s %s %s' % (details['name'], details['version'], details['release'], details['arch_label'], )
  for c in details['providing_channels']:
    print '  %s' % (c, )
